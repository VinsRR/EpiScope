...
from epilens.db import AcademicDB, InMemoryAcademicDB
from epilens.db.mongo_academic_db import MongoAcademicDB
...
from epilens.workspace import state_root
...
# (e.g. `epilens studio` with no external services). Intentionally mirrors
# the CLI's own `.epilens/` convention (`epilens.py`'s `_DEFAULT_INDEX_DIR`
# / `_DEFAULT_DB_BACKUP`) as a duplicated constant, since `services/` must
# not import from the CLI module.
_DEFAULT_LOCAL_INDEX_DIR = state_root() / "index"
_DEFAULT_LOCAL_METADATA_BACKUP = state_root() / "academic_db.json"


@dataclass(frozen=True)
class RuntimeConfig:
    """Configuration for the corpus-backed EpiLens application runtime."""

    strategy_name: str = "grobid"
    mongo_uri: Optional[str] = None
    mongo_db_name: str = "epilens_academic_db"
    qdrant_url: Optional[str] = None
    qdrant_collection: str = "epilens_academic"
...
class EpiLensRuntime:
    """Application-facing facade for corpus-backed EpiLens workflows."""
