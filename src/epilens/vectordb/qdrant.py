...
_SKIP_VERSION_CHECK_ENV = "EPILENS_QDRANT_SKIP_VERSION_CHECK"
...
def _check_qdrant_version_compatibility(
    ...
    """
    if (env(_SKIP_VERSION_CHECK_ENV, "") or "").strip().lower() in {
        "1",
        "true",
        "yes",
    }:
    ...
    def __init__(
        ...
        *,
        ...
        if QdrantClient is None:
            raise ImportError(
                "Qdrant support is not installed. Run "
                "`pip install 'epilens[qdrant]'`."
            )
