...
class OpenRouterClient(UsageTrackingMixin, LLMClient):
    ...
    def __post_init__(self):
        ...
        if OpenAI is None:
            raise ImportError(
                "OpenRouter support is not installed. Run "
                "`pip install 'epilens[openrouter]'`."
            )
...
class OpenAIClient(UsageTrackingMixin, LLMClient):
    ...
    def __post_init__(self):
        ...
        if OpenAI is None:
            raise ImportError(
                "OpenAI support is not installed. Run "
                "`pip install 'epilens[openai]'`."
            )
