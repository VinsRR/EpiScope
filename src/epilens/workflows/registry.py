...
* **Workspace tasks** — JSON files under ``<workspace>/tasks/*.json`` that the
  CLI auto-discovers (and a server can load via ``EPILENS_TASKS_DIR``).
...
from epilens.workflows.classification.schemas import BaseClassificationSchema
...
def validate_task_file(path: str | Path) -> TaskSpec:
    ...
    if the spec is malformed. Intended for ``epilens tasks validate``.
    """
...
def _load_startup_tasks() -> None:
    tasks_dir = env("EPILENS_TASKS_DIR")
    if not tasks_dir:
        return
    try:
        ...
        if loaded:
            logger.info(
                "Loaded %d task(s) from EPILENS_TASKS_DIR=%s", len(loaded), tasks_dir
            )
    except Exception as exc:  # best-effort: a bad tasks dir must not break imports
        logger.warning("Failed to load EPILENS_TASKS_DIR=%s: %s", tasks_dir, exc)
