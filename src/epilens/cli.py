"""Command-line interface for EpiLens."""
...
from epilens.db import InMemoryAcademicDB, MongoAcademicDB
from epilens.rag.embeddings.factory import EmbedderFactory
from epilens.rag.indexing.chunking import (
    FixedSizeChunker,
    NoChunker,
    ParagraphChunker,
    SentenceChunker,
...
from epilens.rag.retrieval.candidates import (
    HybridCandidateRetriever,
    SemanticCandidateRetriever,
    SparseCandidateRetriever,
)
from epilens.rag.retrieval.retriever import Retriever
from epilens.schemas import PaperMetadata, Reference, StructuredSection
from epilens.services import CorpusService, TaskService
from epilens.services.errors import humanize_error
from epilens.settings import AppSettings, env
from epilens.utils.logger import setup_logging
...
from epilens.workflows.registry import (
    TaskSpec,
...
app = typer.Typer(
    help="EpiLens CLI with local-first defaults for indexing, retrieval, classification, and extraction.",
...
def _version_callback(value: bool) -> None:
    if value:
        from epilens import __version__
...
def _root(
    version: bool = typer.Option(
        ...
        "--version",
        "-V",
        help="Show the EpiLens version and exit.",
        callback=_version_callback,
        is_eager=True,
    ),
) -> None:
    """EpiLens CLI with local-first defaults for indexing, retrieval, classification, and extraction."""


_SETTINGS = AppSettings.from_env()
_CLI_ROOT = state_root()
...
def _format_option() -> Any:
    return typer.Option(
        ...
        "--format",
        "-f",
        envvar="EPILENS_OUTPUT_FORMAT",
        help="Output format: 'human' (default) or 'json'. "
        "Falls back to $EPILENS_OUTPUT_FORMAT when set.",
    )
...
def _resolve_value(
    ...
    ``--quality`` wins over ``epilens.toml``, which wins over the
...
def _ollama_setup_hint(model: str) -> str:
    return (
        "To run EpiLens without a cloud API key, use Ollama (free, local):\n"
...
def _resolve_retrieval_mode_for_vectordb(
    ...
    if not vectordb.capabilities().get("sparse"):
        ...
        reason = (
            ...
            "for sparse embeddings (`pip install epilens[local-ml]`)"
        )
    else:
        return retrieval_mode
...
def _transient_retriever_for_path(
    ...
    *,
    ...
    tempdir = tempfile.TemporaryDirectory(prefix="epilens-cli-")
    ...
    *,
    ...
    if metadata is None:
        raise ValueError(
            ...
            "Run `epilens papers` to inspect the available ids."
        )
...
def _check_mongo(uri: str) -> tuple[str, str, str]:
    ...
    try:
        ...
        return "skip", "configured", "Install epilens[server] to verify connectivity."
    try:
...
def _collect_doctor_checks(
    ...
    *,
    ...
    add("ok", "EpiLens version", __version__, section="Environment")
    add(
        "ok" if py_ok else "fail",
        "Python",
        platform.python_version(),
        "" if py_ok else "EpiLens requires Python 3.10 or newer.",
        section="Environment",
    )
    ...
    add(
        ...
        "Compute device",
        resolve_device(),
        "Override with EPILENS_DEVICE=cpu|cuda|mps.",
        section="Environment",
    )
    add(
        "ok",
        "PDF parsing",
        "pdfminer (included, no model download)",
        section="Environment",
    )
    add(
        "ok" if _local_ml_available() else "skip",
        "Optional hi-res parsing",
        "available" if _local_ml_available() else "not installed",
        "Install `epilens[local-ml]` only if you need ML-based layout and "
        "table detection.",
        section="Environment",
    )
    ...
    if resolved_embed_provider == "huggingface" and not _local_ml_available():
        add(
            "warn",
            "Embedding backend",
            f"{resolved_embed_provider} (needs epilens[local-ml])",
            "Install `epilens[local-ml]`, or use a model covered by the "
    ...
    provider_sdk = {
        "gemini": ("google.genai", "epilens[gemini]"),
        "openai": ("openai", "epilens[openai]"),
        "openrouter": ("openai", "epilens[openrouter]"),
        "anthropic": ("anthropic", "epilens[anthropic]"),
    }.get(provider)
    if provider_sdk is not None:
        import importlib.util

        module_name, extra = provider_sdk
        try:
            sdk_available = importlib.util.find_spec(module_name) is not None
        except (ImportError, ModuleNotFoundError):
            sdk_available = False
        add(
            "ok" if sdk_available else ("fail" if require_generation else "warn"),
            "Provider SDK",
            "installed" if sdk_available else "not installed",
            "" if sdk_available else f"Install it with `pip install '{extra}'`.",
            section="LLM",
        )
    if provider == "ollama":
        if probe:
        ...
        add(
            "ok" if has_key else ("fail" if require_generation else "warn"),
            key_env,
        ...
        add(
            ...
            "Set EPILENS_LLM_PROVIDER to one of: gemini, openai, openrouter, anthropic, ollama.",
            section="LLM",
        )
    ...
    if probe:
        ...
        add(
            ...
            if ok
                ...
                "'pdfminer' loader works without it."
            ),
            section="Services",
        )
    ...
    if ws is not None:
        ...
        add(
            ...
            "Workspace",
            "none discovered",
            "Optional. Run `epilens init <name>` to create one.",
            section="Workspace",
        )
...
def doctor(
    ...
    workspace: Optional[Path] = typer.Option(
        ...
        "--workspace",
        "-w",
        help="Workspace directory containing epilens.toml.",
    ),
...
def quickstart(
    workspace: Optional[Path] = typer.Option(
        ...
        "--workspace",
        "-w",
        help="Workspace directory to check at the end (see `epilens init`).",
    ),
    probe: bool = typer.Option(
        False,
        "--probe/--no-probe",
        help="Also probe optional local services over the network.",
    ),
) -> None:
    ...
    or make any model/API calls itself. Run `epilens doctor` any time
    afterward to re-check your environment.
    """
    import dotenv

    try:
        typer.secho("EpiLens quickstart", bold=True)
        ...
        try:
            ...
            _abort(f"Invalid choice {raw_choice!r}. Run `epilens quickstart` again.")
        ...
        dotenv.set_key(str(env_path), "EPILENS_LLM_PROVIDER", provider)
        typer.echo(f"\nSet EPILENS_LLM_PROVIDER={provider} in {env_path}.")
        ...
        if provider == "ollama":
            if probe:
                ...
                typer.echo("Skipping the Ollama connection check (use --probe to run it).")
            ...
            dotenv.set_key(str(env_path), "EPILENS_LLM_MODEL", _OLLAMA_SMALL_MODEL)
        else:
        ...
        checks, any_fail = _collect_doctor_checks(
            probe=probe,
            workspace=workspace,
        ...
        if any_fail:
            ...
            typer.echo(
                '  epilens ask "What data sources were used?" --path /path/to/paper.pdf'
            )
    except Exception as exc:
        _abort_exc(exc)
...
def init_workspace(
    ...
    force: bool = typer.Option(
        False,
        "--force",
        help="Overwrite an existing epilens.toml in the workspace directory.",
    ),
    output_format: OutputFormat = _format_option(),
) -> None:
    """Create an EpiLens workspace with local-first defaults."""
    try:
...
def inspect_document(
    ...
    loader: LoaderKind = typer.Option(
        LoaderKind.pdfminer,
        "--loader",
        help="Parsing backend. pdfminer is the lightweight local default.",
    ),
    output_format: OutputFormat = _format_option(),
) -> None:
...
def index(
    ...
    loader: LoaderKind = typer.Option(
        LoaderKind.pdfminer,
        "--loader",
        help="Parsing backend. pdfminer needs no model or external service.",
    ),
    ...
    try:
        ...
        if use_managed_corpus and workspace_config is not None:
            ...
            mutation_lock = (
                FileLock(str(workspace_lock_path(workspace_config)), timeout=60)
...
def ask(
    ...
    loader: LoaderKind = typer.Option(
        LoaderKind.pdfminer,
        "--loader",
        help="Parsing backend. Use `unstructured` or `grobid` for richer structure.",
        rich_help_panel="Advanced (retrieval internals)",
    ),
    embed_model: str = typer.Option(
        _DEFAULT_EMBED_MODEL,
    ...
    try:
        ...
        try:
            ...
            typer.secho(
                ...
                "of a generated answer. Run `epilens quickstart` to enable "
                "generated answers.",
                fg=typer.colors.YELLOW,
                err=True,
            )
...
def classify(
    ...
    classifier_kind: str = typer.Option(
        "data_accessibility",
        "--classifier-kind",
        help="Built-in or workspace/declarative task key (see `epilens tasks`).",
    ),
...
def _run_task_wizard(kind: str, workspace_config: Optional[WorkspaceConfig]) -> None:
    ...
    if workspace_config is not None and output_path.is_relative_to(default_dir):
        ...
        typer.echo(f"Run it with: epilens {example_cmd} --task-file {output_path}")
...
def tasks(
    ...
    workspace: Optional[Path] = typer.Option(
        ...
        "--workspace",
        "-w",
        help="Workspace directory containing epilens.toml (used by 'list' and "
        "'new --interactive').",
    ),
    output_format: OutputFormat = _format_option(),
) -> None:
    ...
    \b
    Examples:
      epilens tasks                                  # list all kinds
      epilens tasks new --kind classifier            # print a scaffold
      epilens tasks new --kind miner > my.json       # save it
      epilens tasks new --kind classifier --interactive  # answer prompts instead
      epilens tasks validate --task-file my.json     # lint without running
    """
    try:
        if action == "list":
            workspace_config = _optional_workspace(workspace)
...
def serve(
    ...
    try:
        ...
        _abort(
            f"The API server is not installed: {exc}. "
            "Run `pip install 'epilens[server]'`."
        )
    uvicorn.run("epilens.api:app", host=host, port=port, reload=reload)
...
def studio(
    workspace: Optional[Path] = typer.Option(
        ...
        "current directory's local `.epilens/` store.",
    ),
    ...
    try:
        ...
        _abort(
            ...
            "`studio` needs both the API and UI extras: install epilens[all]."
        )
    try:
    ...
    try:
        ...
        if workspaces_root is not None:
            ...
            configured_root = env("EPILENS_WORKSPACES_ROOT")
            managed_root = (
                Path(configured_root).expanduser().resolve()
                if configured_root
                else (state_root(Path.home()) / "workspaces").resolve()
            )
        child_env["EPILENS_WORKSPACES_ROOT"] = str(managed_root)
        ...
        if workspace_config is not None:
            ...
            child_env["EPILENS_ACTIVE_WORKSPACE"] = workspace_config.root.name
            index_dir = workspace_config.resolve_path(workspace_config.index_dir)
            metadata_path = workspace_config.resolve_path(
                workspace_config.metadata_path
            )
            if not env("QDRANT_URL"):
                child_env["EPILENS_LOCAL_INDEX_DIR"] = str(index_dir)
            if not env("MONGO_URI"):
                child_env["EPILENS_LOCAL_METADATA_BACKUP"] = str(metadata_path)
        if (
            workspace_config is not None
        ...
        typer.secho("Starting EpiLens studio", bold=True)
        ...
        ui_app_path = resources.files("epilens.ui").joinpath("streamlit_app.py")
        ui_env = {**child_env, "EPILENS_API_BASE_URL": api_url}
        ui_proc = subprocess.Popen(
            [
                ...
                child_env.get("EPILENS_UPLOAD_LIMIT_MB", "100"),
