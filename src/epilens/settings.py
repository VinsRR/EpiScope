...
class AppSettings:
    ...
    def from_env(cls) -> "AppSettings":
        ...
        return cls(
            ...
            llm_provider=(env("EPILENS_LLM_PROVIDER", "gemini") or "gemini").lower(),
            llm_model=env("EPILENS_LLM_MODEL", "gemini-2.5-flash")
            or "gemini-2.5-flash",
            embed_provider=(env("EPILENS_EMBED_PROVIDER", "auto") or "auto").lower(),
            cross_encoder_model=env("CROSS_ENCODER_MODEL"),
            ollama_host=env("OLLAMA_HOST", "http://localhost:11434")
            or "http://localhost:11434",
            api_host=env("EPILENS_API_HOST", "0.0.0.0") or "0.0.0.0",
            api_port=int(port_raw) if port_raw else 8000,
            api_base_url=env("EPILENS_API_BASE_URL", "http://localhost:8000")
            or "http://localhost:8000",
            log_level=(env("EPILENS_LOG_LEVEL", "INFO") or "INFO").upper(),
            local_index_dir=env("EPILENS_LOCAL_INDEX_DIR"),
            local_metadata_backup=env("EPILENS_LOCAL_METADATA_BACKUP"),
        )
