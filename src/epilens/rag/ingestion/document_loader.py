"""Document loading utilities for EpiLens.
...
consisting of :class:`~epilens.core.blueprints.data_blueprints.StructuredSection`
instances accompanied by minimal metadata.  The goal is to make
documents available to indexers (e.g., :class:`~epilens.index.paper_indexer.PaperIndexer`)
...
instantiate a loader based on a string key (for example, ``"pdfminer"``,
``"unstructured"``, or ``"grobid"``). Additional loaders can be registered by extending the
factory.
"""
...
from epilens.schemas import StructuredSection, PaperMetadata, Reference
from epilens.db.academic_db import AcademicDB
    ...
    def _load_pdf(self, path: Path) -> Tuple[List[StructuredSection], PaperMetadata]:
        ...
        try:
            ...
            logger.warning(
                ...
                "`pip install epilens[local-ml]`). Falling back to a "
    ...
    def _load_pdf_with_pdfminer(
        ...
        if not all_lines:
            raise RuntimeError(
                ...
                f"or install epilens[local-ml] for `--loader unstructured` hi_res parsing."
            )
...
class PdfMinerDocumentLoader(UnstructuredDocumentLoader):
    """Deterministic, lightweight PDF loader included in the base install.

    Text and Markdown handling is inherited from ``UnstructuredDocumentLoader``;
    PDFs always take the pdfminer path, even when the optional ML stack happens
...
class GrobidDocumentLoader(AbstractDocumentLoader):
    ...
    def __init__(self, grobid_url: Optional[str] = None) -> None:
        ...
        try:
            from epilens.rag.ingestion.local_grobid_client import GrobidClient
...
class DocumentLoaderFactory:
    ...
    LOADER_REGISTRY: Dict[str, Callable[..., AbstractDocumentLoader]] = {
        "pdfminer": PdfMinerDocumentLoader,
    ...
    def get_loader(cls, name: str, **kwargs: Any) -> AbstractDocumentLoader:
        ...
        Args:
            name: Name of the loader (``"pdfminer"``, ``"unstructured"``, or
                ``"grobid"``).
