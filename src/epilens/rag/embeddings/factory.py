...
_LOCAL_ML_HINT = (
    ...
    "`pip install epilens[local-ml]`, or use a model covered by the "
...
class EmbedderFactory:
    ...
    2. the ``EPILENS_EMBED_PROVIDER`` environment variable, else
    ...
    def get_embedder(
        ...
        **kwargs,
        ...
        resolved = (
            (provider or env("EPILENS_EMBED_PROVIDER") or "auto")
            .strip()
            .lower()
        )
        if resolved == "auto":
    ...
    def _detect_provider(model_name: str) -> str:
        ...
        if "/" in name and not name.startswith("models/"):
            ...
            # provider="huggingface" (needs epilens[local-ml]).
        ...
        raise ValueError(
            ...
            f"Pass provider=... or set EPILENS_EMBED_PROVIDER to one of: "
            f"{', '.join(sorted({*EMBEDDER_PROVIDERS, 'huggingface'}))}."
        )

    @staticmethod
    def get_sparse_embedder(model_name: str, **kwargs) -> SparseEmbedder:
        """Factory method to get a sparse embedder instance (needs epilens[local-ml])."""
    ...
    def get_late_embedder(model_name: str, **kwargs) -> LateEmbedder:
        """Factory method to get a late-interaction embedder instance (needs epilens[local-ml])."""
