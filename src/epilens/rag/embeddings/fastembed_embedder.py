...
# overlaps with models EpiLens has historically shipped as defaults. This is
    ...
    def __init__(
        ...
        if model not in FASTEMBED_SUPPORTED_MODELS:
            raise ValueError(
                ...
                f"Pass provider='huggingface' (needs `pip install epilens[local-ml]`) "
        ...
        try:
            ...
            raise ImportError(
                ...
                "fastembed`, or `pip install epilens[local-ml]` for the "
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not initialize the local embedding model {model!r}. "
                "The first retrieval or indexing run downloads and caches this "
                "model (about 90 MB for the default). Check your internet "
                "connection and retry; later runs can work from the cache. "
                "`epilens inspect FILE` remains fully offline and needs no model."
            ) from exc
