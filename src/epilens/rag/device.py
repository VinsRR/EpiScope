...
Override with ``EPILENS_DEVICE=cpu|cuda|mps`` or by passing ``device=`` to the
embedder / reranker.
"""
...
def resolve_device(requested: Optional[str] = None) -> str:
    ...
    Resolution order: explicit ``requested`` → ``EPILENS_DEVICE`` → ``"auto"``.
    """
    pref = (requested or env("EPILENS_DEVICE") or "auto").strip().lower()
    ...
    try:
        ...
        if capability < min(built):
            logger.warning(
                ...
                "(built for %s); using CPU instead. Set EPILENS_DEVICE=cuda to "
                "force the GPU.",
                capability,
                sorted(built),
            )
