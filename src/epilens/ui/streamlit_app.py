...
from epilens.ui.api_client import StudioApiClient

dotenv.load_dotenv()

DEFAULT_API_BASE_URL = env("EPILENS_API_BASE_URL", "http://localhost:8000")
...
st.set_page_config(page_title="EpiLens Studio", page_icon="🔬", layout="wide")
...
def home_page() -> None:
    ...
    try:
        ...
        st.error(f"The EpiLens API is not reachable: {exc}")
        st.code("epilens studio", language="bash")
        return
    ...
    with st.expander("Setup diagnostics"):
        diagnostics = [
            {"Component": "API", "Status": "Ready", "Detail": "Connected"},
            {
                "Component": "Parser",
                "Status": "Ready",
                "Detail": checks.get("parser_backend", "pdfminer"),
            },
            {
    ...
    if workspace_items:
        ...
        st.caption(
            ...
            "`epilens studio --workspace /path/to/workspace`."
        )
...
def task_builder_page() -> None:
    ...
    if advanced:
        ...
        if kind == "classifier":
            ...
            st.caption(
                "Examples are optional, but useful: EpiLens uses them as retrieval "
...
def settings_page() -> None:
    ...
    if not workspace["indexed"]:
        with st.form("index-settings"):
            loader_options = ["pdfminer", "unstructured", "grobid"]
            loader = st.selectbox(
...
test_page = env("EPILENS_STUDIO_TEST_PAGE")
if test_page:
