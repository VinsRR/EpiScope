...
class BackendConfig(BaseModel):
    ...
    mongo_uri: Optional[str] = Field(default_factory=lambda: _env("MONGO_URI"))
    mongo_db_name: str = Field(default_factory=lambda: _env("MONGO_DB_NAME", "episcope_academic_db") or "episcope_academic_db")
    qdrant_url: str = Field(default_factory=lambda: _env("QDRANT_URL", "http://localhost:6333") or "http://localhost:6333")
    qdrant_collection: str = Field(default_factory=lambda: _env("QDRANT_COLLECTION", "episcope_academic") or "episcope_academic")
    llm_provider: Literal["gemini", "openai", "openrouter", "ollama"] = Field(
        default_factory=lambda: (_env("EPISCOPE_LLM_PROVIDER", "gemini") or "gemini").lower()
    )
    llm_model: str = Field(default_factory=lambda: _env("EPISCOPE_LLM_MODEL", "gemini-2.5-flash") or "gemini-2.5-flash")
    ...
    cross_encoder_model: Optional[str] = Field(default_factory=lambda: _env("CROSS_ENCODER_MODEL"))
        ...
        "llm_api_key_configured": bool(
            {
                ...
                "ollama": _env("OLLAMA_HOST", "http://localhost:11434"),
